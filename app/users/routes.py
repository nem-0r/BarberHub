...
from app.users.auth_verification import (
...
def _issue_tokens(response: Response, user: User) -> dict:
    """Return access token in body and set refresh token cookie."""
...
async def get_my_stats(
    ...
    """Role-aware stats for the current user (bookings, spend, etc.)."""
...
async def login(
    ...
    # Reject OAuth-only accounts; dummy verify equalizes timing to prevent enumeration.
...
async def login_with_google(
    ...
    """Sign in or register via a Google id_token."""
    ...
    try:
        payload = await _asyncio.wait_for(
            _asyncio.to_thread(_verify_blocking),
            timeout=5.0,
        )
    ...
    if not email or not google_sub:
        raise HTTPException(
            status_code=401, detail="Google credential missing required claims"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Could not provision Google account."
        )
...
async def delete_user(
    request: Request, user_id: uuid.UUID, session: AsyncSession = Depends(get_session)
):
...
async def resend_verification(
    ...
    """Re-send the email verification link (always returns generic 200)."""
