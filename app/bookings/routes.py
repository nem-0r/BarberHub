...
@router.get(
    "/salon/{salon_id}",
    response_model=List[BookingRead],
    dependencies=[Depends(owner_admin)],
)
...
async def get_booking(
    ...
    if (
        str(current_user.id) != str(booking.client_id)
        ...
        is_staff = current_user.role == UserRole.staff and str(staff.user_id) == str(
            current_user.id
        )
        is_owner = (
            current_user.role == UserRole.owner
            and salon
            and str(salon.owner_id) == str(current_user.id)
        )
...
async def create_booking(
    ...
    if (
        str(current_user.id) != str(data.client_id)
        and current_user.role != UserRole.admin
    ):
        raise HTTPException(
            status_code=403, detail="Not authorized to book for another user"
        )
...
async def cancel_booking(
    ...
    if str(current_user.id) != str(booking.client_id) and current_user.role not in [
        UserRole.owner,
        UserRole.admin,
    ]:
        raise HTTPException(
