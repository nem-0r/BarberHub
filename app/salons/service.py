...
def calculate_salon_status(salon: Salon):
    """Return (is_open, close_time) based on operating_hours and salon timezone."""
...
async def get_salon_stats(salon_id: uuid.UUID, session: AsyncSession):
    # Naive UTC — Booking.start_time is TIMESTAMP WITHOUT TIME ZONE.
...
async def update_salon(
    salon_id: uuid.UUID, data: SalonUpdate, session: AsyncSession, current_user: User
) -> Optional[SalonRead]:
    salon = await session.get(Salon, salon_id)
    if not salon:
        return None
...
async def delete_salon(
    salon_id: uuid.UUID, session: AsyncSession, current_user: User
) -> bool:
    salon = await session.get(Salon, salon_id)
    if not salon:
        return False
