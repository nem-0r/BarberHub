...
async def get_available_slots(
    ...
    service_id: uuid.UUID = Query(..., description="Service UUID to determine slot duration"),
...
async def create_schedule(
    ...
    if current_user.role == UserRole.staff and str(staff.user_id) != str(current_user.id):
        raise HTTPException(status_code=403, detail="Staff can only manage their own schedule")
    if current_user.role == UserRole.owner:
        salon = await session.get(Salon, staff.salon_id)
        if str(salon.owner_id) != str(current_user.id):
            raise HTTPException(status_code=403, detail="Not authorized to manage schedule for this salon's staff")
