...
async def create_service(
    data: ServiceCreate, session: AsyncSession, current_user: User
) -> Service:
    ...
    if current_user.role != UserRole.admin and str(salon.owner_id) != str(
        ...
        raise HTTPException(
            status_code=403, detail="Not authorized to add services to this salon"
        )
...
async def update_service(
    ...
    if current_user.role != UserRole.admin and str(salon.owner_id) != str(
        ...
        raise HTTPException(
            status_code=403, detail="Not authorized to modify this service"
        )
...
async def delete_service(
    service_id: uuid.UUID, session: AsyncSession, current_user: User
) -> bool:
    service = await session.get(Service, service_id)
    if not service:
        return False
    ...
    links = (
        await session.exec(
            select(StaffService).where(StaffService.service_id == service_id)
        )
    ).all()
    for link in links:
